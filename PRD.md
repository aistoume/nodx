...
> 版本: v0.9（自动递进引擎：项目经理 PM + Auto-Step 默认 + 预算 $5 封顶 + 每层路径预览 + CBR 复用）| 作者: LaoMo + AI | 日期: 2026-06-02
...
- 用户采纳 Local Max 后可选触发**自动递进引擎（§3.19）**：项目经理 PM 审视结论是否够原子，按需自动 spawn 子话题继续推进到实操级别
...
### 3.19 自动递进引擎（Auto-Recursion Engine）

**问题**：现有专家组协议解决了"想得深"（Local Maximum），但**不主动推进**——每次得出结论后等用户决定深挖哪条线、何时停。结果是：用户得到了带框架的施工蓝图，但**蓝图上某些房间仍是粗略草图**，到不了可执行层级。

本节加一个**项目经理（Project Manager, PM）**角色，在每次 Local Maximum 被采纳时主动接管，判断"够不够原子"，并按可行性自动 spawn 下一层子话题继续推进，直到方案落到实操级别。

> **理论锚点**：保持"用户主导方向，AI 主导深度"——PM 不替用户拍板分叉，只**把决策结构化展示**并按预设策略推进；用户作为 Chair 始终能打断、改向、撤回。

**PM 做的三件事**

每次 Local Maximum 被采纳后触发：

1. **评估 atomicity**：当前结论够"原子"吗（谁 / 何时 / 产出物 / 可验收）
2. **分流状态**：
   - `atomic_complete` — 够原子，停
   - `needs_deepening` — 还需深挖（识别卡点 / 分叉选项）
   - `needs_real_world_data` — 缺真实世界数据，靠想没用 → 标为外部 atomic action 后停
   - `multi_path_choice` — 多个并列方案需择一
3. **出 NextMovePlan**：候选子话题按可行性排序，每个带 5 维 breakdown + 推荐动作

**三种推进模式**

| 模式 | 行为 |
|---|---|
| 🔵 Pilot | PM 出方案，用户挑 1–N 个子话题手动 spawn |
| 🟢 **Auto-Step（默认）** | PM 自动 spawn topPick + 跑专家组，**每层结束等用户确认**再下一层 |
| 🟡 Auto-Run | 沿 topPick 全自动递归到底（atomic / 预算耗尽 / 深度耗尽才停）|

**Auto-Run 不是默认**——抵抗"AI 代劳"侵蚀必须保留这条边界，需用户每次显式选 + 二次确认 + 输入预算。

**必备的硬封顶（防失控）**

- **预算上限**：每个根问题默认 **$5**（Settings 可改；后续按 plan 分层定价）
- **深度上限**：默认 4 层
- **每层"路径预览"卡片**：UI **强制**弹出当前根 → 当前层的推理链快照，让用户看见 AI 在走哪条路
  - Auto-Step：必须点确认才下一层
  - Auto-Run：弹 3 秒可关，但点击"打回上一层"会回退并让用户挑别的候选
- 任一上限触发 → 立刻停 + 报告本次推进的：花了多少钱、深到几层、创建了哪些子话题

**与 CBR 索引的连接（关键省钱）**

每个 PM 决定 spawn 子话题前，**先查 CBR 索引**：

- 命中相似案例（adapter 给出 high similarity）→ 跳过专家组，走 **Fork & Adapt** 复用
- 未命中 → 跑专家组并入库

这个循环让案例库越用越聚集，长期 Auto-Run 成本不断下降。

**与既有架构的串联**

| 已有能力 | 在 PM 流程中的角色 |
|---|---|
| 专家组协议（§3.14）| 每个 spawn 出的子话题跑专家组 |
| Local Maximum 结构 | PM 的输入 |
| 卡点 `open_question`（§3.12）| `needs_deepening` 的关键信号 |
| 原子化检查器（§3.6）| `atomicityScore` 复用其逻辑 |
| 子话题 spawn（§3.7）| 自动 spawn 的底层 |
| CBR 索引（§3.16）| 子话题 spawn 前先查复用 |
...
[ 专家组 Local Max ]
        ↓
用户点「采纳并推进」（新按钮，在现有「采纳」旁）
        ↓
选模式（默认 Auto-Step）+ 预算上限（$5）
        ↓
[项目经理 PM (Sonnet)] 评估当前结论
        ↓
  atomicityScore ≥ 0.7  →  STOP（标完成）
  needs_real_world_data →  STOP（标为外部 atomic action）
  否则                    →  出 NextMovePlan
        ↓
[路径预览卡片] 用户审阅推理链 + topPick
        ↓
  Auto-Step：等用户确认 / Auto-Run：3s 自动放行
        ↓
[先查 CBR] 命中相似 → Fork & Adapt
           未命中    → spawn 子话题 + 跑专家组
        ↓
子话题 Local Max → 回到 PM 评估 → 递归
        ↓
停止条件：atomic / 预算耗尽 / 深度耗尽 / 用户中断
```
...
| 角色 | 触发 | 任务 |
|---|---|---|
| 项目经理 PM（Sonnet） | Local Maximum 被采纳并选择"推进" | 评估 atomicity；分流状态；出 NextMovePlan |
| 可行性评分员（Haiku） | PM 评估每个候选子话题时 | 5 维评分（resourceCost / timeToResolve / decisionRisk / value / dependencies），PM 的子调用以省 token |
...
type FeasibilityBreakdown = {
  resourceCost: number;          // 0–1，越低越好
  timeToResolve: number;          // 0–1，越快越好
  decisionRisk: number;           // 0–1，越低越好
  value: number;                  // 0–1
  dependencies: string[];         // 依赖什么
};

type ChildCandidate = {
  title: string;                  // 子话题候选标题
  sourceOpenQuestion?: string;    // 哪个卡点衍生
  sourceOptionChoice?: string;    // 哪个分叉衍生
  feasibilityScore: number;       // 综合 0–1
  breakdown: FeasibilityBreakdown;
  recommendedAction: 'spawn_and_run' | 'spawn_only'
                   | 'skip' | 'flag_as_real_world_action';
};

type NextMovePlan = {
  id: string;
  topicId: string;                // 来源 Topic（被评估的那个）
  status: 'atomic_complete' | 'needs_deepening'
        | 'needs_real_world_data' | 'multi_path_choice';
  atomicityScore: number;         // 0–1
  whatsMissing: string[];          // 缺什么才算原子
  childCandidates: ChildCandidate[];
  topPick?: string;               // 推荐子话题的 title
...
type AutoRecursionRun = {
  id: string;
  rootTopicId: string;
  mode: 'pilot' | 'auto_step' | 'auto_run';
  budgetUsd: number;              // 预算上限（默认 5.0）
  depthLimit: number;             // 深度上限（默认 4）
  startedAt: number;
  endedAt?: number;
  status: 'running' | 'paused_by_user' | 'completed'
        | 'budget_exhausted' | 'depth_exhausted' | 'hit_real_world_block';
  totalSpentUsd: number;
  maxDepthReached: number;
  spawnedTopicIds: string[];      // 这次推进新建的全部 Topic
  interruptions: {
    topicId: string;
    action: 'redirected' | 'paused' | 'rolled_back';
    at: number;
  }[];
};

// Topic 扩展字段（不重复列完整定义，仅标注新增）：
//   generatedByAutoRecursionRunId?: string  // 关联回 Run，方便回滚 / 可视化路径树
//   autoRecursionDepth?: number              // 在该 Run 里的深度
//   parentNextMovePlanId?: string            // 由哪个 PM 计划 spawn 出来的
```
...
- **自动递进引擎（§3.19）+ 项目经理 PM**：Auto-Step 默认 + 预算 $5 + 深度 4 封顶 + 每层路径预览 + CBR 复用
- 移动端（React Native）
- 反向引用
...
| 自动递进走偏（PM 自动选了路径，几层都没回头） | 每层强制弹路径预览卡片；Auto-Step 必须点确认；Auto-Run 可"打回上一层"换候选；Pilot 始终可选 |
| 递进成本爆炸（5 层 × 专家组 ≈ $15） | 单次 Auto-Run 预算上限默认 $5；硬封顶 + 实时累计监控；超限立刻停 + 报告 |
| 不该深挖的卡点被深挖（需真实世界调研而非更多思考） | PM 识别 `needs_real_world_data` 状态，标为外部 atomic action 直接停 |
| AI 代劳侵蚀（用户失去 Chair 感） | Auto-Run 不是默认；每次都需显式选 + 二次确认；用户随时可中断 / 改向 / 回退 |
